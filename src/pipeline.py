...
# 수집 날짜 하한: '오늘 −LOOKBACK_DAYS일 00:00(로컬)' 이후 발행분만 수집.
# 기본 1 → 어제 0시부터 지금까지(= 어제+오늘). 2로 바꾸면 그제부터.
LOOKBACK_DAYS = 1


def _cutoff():
    """수집 대상 발행시각 하한(로컬 타임존, aware datetime)."""
    today_midnight = datetime.now().astimezone().replace(
        hour=0, minute=0, second=0, microsecond=0
    )
    return today_midnight - timedelta(days=LOOKBACK_DAYS)
...
def _build_stats(entries, feed_reports):
...
def run(progress_cb=None, log_cb=None):
    ...
    log(f"수집 범위: {cutoff:%Y-%m-%d %H:%M} 이후 발행분만 (최근 {LOOKBACK_DAYS}일).")
    ...
    for idx, feed in enumerate(all_feeds, 1):
        ...
        for e in entries:
            ...
            if not use_dates:
                ...
                continue
            if pub is not None:
                # 날짜 하한(최근 1일) + 증분(지난 실행 이후) 둘 다 만족해야 신규
                if pub >= cutoff and (last_max is None or pub > last_max):
                    fresh_list.append(e)
                if run_max is None or pub > run_max:
                    run_max = pub
            else:
                # 날짜가 없으면 '최근 1일 이내'임을 보장할 수 없어 제외(요청 규칙 준수)
                undated += 1
        ...
        scope = f"최근 {LOOKBACK_DAYS}일 " if use_dates else ""
    ...
    report_path = REPORTS_DIR / f"report_{generated_at:%Y-%m-%d}.html"
    if report_path.exists():
        report_path = REPORTS_DIR / f"report_{generated_at:%Y-%m-%d_%H%M}.html"

    stats = _build_stats(new_entries, feed_reports)
